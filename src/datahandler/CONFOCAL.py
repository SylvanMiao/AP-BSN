...
class Confocal(DenoiseDataSet):
    ...
    def _scan(self):
        ...
        assert len(self.img_paths) > 0, 'No PNG images found in %s'%dataset_path
    ...
    def _load_data(self, data_idx):
        ...
        if img.ndim > 2:
            img = img[:, :, 0]  # take first channel (handles RGB, RGBA, etc.)
        assert img.ndim == 2, \
...
class prep_confocal(DenoiseDataSet):
    ...
    def _scan(self):
        ...
        # self. dataset_path = '../../../Dataset/T3/all_crop_pix512_pix1024_300pics'  # 如果跳过prep直接用原图, 取消这行注释并注释上一行
